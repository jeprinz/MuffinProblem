...
def scott(muffins, majors, minors):
    ...
    L = math.ceil(1/(Nm/Ns2 + 1 - Ps2))

    #Given the above assumptions and value of L, we can calculate the number of L-length chains there are, a,
    #and the number of (L-1)-length chains, b. We can get two equations by looking at
    ...
    #The L-chains will be majors, unless Ps2 = 2 and Vm >= Vs2. Its unclear to me if either of those conditions are possible,
    #but just to be safe I code them below

    #The following are the correct values of N, V, P for the L-chains and (L-1)-chains
    newMajors = (a, (L*Ps2 - (L-1))*Vm - L*Vs2, L*Ps2 - 2*(L-1))
    newMinors = (b, (L*Ps2 - (L-2))*Vm - (L-1)*Vs2, (L-1)*Ps2 - 2*(L-2))

    if Ps2 = 2 and Vm >= Vs2:
        newMajors, newMinors = newMinors, newMajors
